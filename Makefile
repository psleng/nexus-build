...
help:
	...
	@echo
	@echo '    make targ-perle-igos       # Perle AM64x IGOS board with engineering signing keys'
	@echo '    make targ-perle-igos-prod  # Perle AM64x IGOS board with production signing keys'
...
IGOS_PKGS='igos-cloud-proxy flask-react-web apnscripts psl-progs saml-sso iolan_apps vyos-1x vyatta-bash vyos-user-utils \
           ...
           ssh-tpm-agent tacacs live-boot tpm2-openssl grub2'
...
# target build: Perle IGOS board using engineering signing gpgkeys
targ-perle-igos:
	@test `arch` = aarch64 || { echo "WARNING: building for this target on this machine (`arch`) is not supported" >&2; }
	@echo "$(AUTOGEN)" > $(DEFS)
	@echo BUILDTARG=perle-igos >> $(DEFS)
	@echo BUILDTYPE=bookworm-am64xx-igos >> $(DEFS)
	@echo IGOS_PKGS="$(IGOS_PKGS)" >> $(DEFS)
	@echo '### Build is Using engineering signing gpgkeys ###'
	rm -rf gpgkeys
	git clone git@github.com:Perle-Systems-Limited/gpgkeys

# target build: Perle IGOS board using production signing gpgkeys
targ-perle-igos-prod:
	@test `arch` = aarch64 || { echo "WARNING: building for this target on this machine (`arch`) is not supported" >&2; }
	@echo "$(AUTOGEN)" > $(DEFS)
	@echo BUILDTARG=perle-igos >> $(DEFS)
	@echo BUILDTYPE=bookworm-am64xx-igos >> $(DEFS)
	@echo IGOS_PKGS="$(IGOS_PKGS)" >> $(DEFS)
	@echo '### Build is Using production signing gpgkeys ###'
	rm -rf gpgkeys
	mkdir -p gpgkeys
	cp ~/gpgkeys/* gpgkeys
...
# Clean everything.
clean: mostlyclean
	sudo rm -rf images dfu-images iso-images gpgkeys
